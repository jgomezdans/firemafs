...
class DEMC_sampler:
  ...
  code-block:: python
	...
	"""
	def __init__ ( self, num_population, CR=1.0, F=2.38, pSnooker=0.1, pGamma1=0.1, n_generations=1000, n_thin=5, n_burnin=200, eps_mult=0.1, eps_add=0) :
		"""
	...
	def demc_zs ( self,  Z):
		...
		#Start of main loop
		for iteration in xrange(self.n_generations):
			...
			if iteration%self.n_thin==0:
				...
				#print "\t",X[0,:]
				#print "\t",X[1,:]
		return (Z[:,:int(m0+self.num_population*numpy.floor(self.n_burnin/self.n_thin))], accept/self.num_population)
...
if __name__=="__main__":
	...
	pylab.figure();pylab.hist(Z_out[0,:],bins=20);pylab.hist(Z_out[1,:],bins=20);
